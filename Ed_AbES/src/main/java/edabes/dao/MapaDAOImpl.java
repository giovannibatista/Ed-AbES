...
import edabes.entidades.MapaNivel;
...
public class MapaDAOImpl implements MapaDAO {
	 ...
	 * @return boolean mapaExcluido - True caso o mapa tenha sido
	...
	public void excluirObjetosDoMapa(int idMapa) {
		...
		try {
			session = sessionFactory.getCurrentSession();
			query = session
					.createQuery("delete MapaObjeto where ID_MAPA = :idMapa");
			query.setParameter("idMapa", idMapa);
			int result = query.executeUpdate();
	...
	public Mapa buscaMapaPorId(int mapaId) {
		...
		try {
			session = sessionFactory.getCurrentSession();
			query = session
					.createQuery("from Mapa where ID_MAPA = :mapaId");
	...
	public Mapa buscaMapaPorDataCriacao(Date dataCriacao) {
		...
		try {
			...
			session = sessionFactory.getCurrentSession();
			query = session
					.createQuery("from Mapa where DT_CRIACAO LIKE :dataCriacao");
	 ...
	 * @return int mapaId - Identificador do mapa. Sera -1 caso ocorra alguma falha
	 */
	 ...
	 * @param Mapa caracteristicasMapa - Objeto com as caracteristicas do mapa
	 * @return boolean caracteristicasSavlas - True caso o mapa tenha sido salvo com sucesso
	 */
	 ...
	 * que possuem informacoes dos mapas
	 */
	 ...
	 * @return ArrayList<ListarMapaDTO> listaMapasDoUsuario - Lista com ListarMapaDTO 
	 * que possuem informacoes dos mapas
	 */
	 ...
	 * @param int idUsuario - Identificador do usuario
	 * @return ArrayList<ListarMapaDTO> listaMapasImportadosDoUsuario - Lista com ListarMapaDTO 
	 * que possuem informacoes dos mapas
	 */
	 ...
	 * @param int idMapa - Identificador do mapa
	 * @return ArrayList<MapaObjeto> listaMapaObjetos - Lista do MapaObjetos do mapa
	 */
	 ...
	 * @param int idMapa - Identificador do mapa
	 * @return HashMap<Integer, Objeto> returnObject - Hashmap contendo o id do objeto e as informacoes do mesmo
	 */
	 ...
	 * @param int idMapa - Idntificador do mapa
	 * @return boolean mudancasEfetuadas - True caso as mudancas tenham sido salvas com sucesso
	 */
	 ...
	 * @param int idMapa - Identificador do mapa
	 * @return boolean operacaoRealizada - True caso os objetos tenham sidos salvos
	 * com sucesso
	 */
	 ...
	 * @param int idUsuario - Identificador do usuario
	 * @param String nomeMapa - Nome do mapa
	 * @return boolean mapaImportado - True caso o mapa ja tenho sido importado pelo usuario
	 */
	 ...
	 * @return int idMapaImportado - Identificador do mapa importado. Sera -1 caso ocorra alguma falha
	 */
